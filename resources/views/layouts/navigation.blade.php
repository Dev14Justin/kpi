    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16 items-center relative">
            ...
            <div class="flex items-center absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 space-x-5 md:space-x-8">
                <x-nav-link :href="route('influencers.index')" :active="request()->routeIs('influencers.index')" wire:navigate class="gap-2 px-3">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" />
                    </svg>
                    <span class="hidden sm:inline">{{ __('Influenceurs') }}</span>
                </x-nav-link>
                <x-nav-link :href="route('ranking.index')" :active="request()->routeIs('ranking.index')" wire:navigate class="gap-2 px-3">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <span class="hidden sm:inline">{{ __('Classement') }}</span>
                </x-nav-link>
                <x-nav-link :href="route('enterprises.index')" :active="request()->routeIs('enterprises.index')" wire:navigate class="gap-2 px-3">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
                    </svg>
                    <span class="hidden sm:inline">{{ __('Entreprises') }}</span>
                </x-nav-link>
            </div>
